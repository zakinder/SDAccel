...
unsigned int getPrintfBufferSize(std::vector<unsigned int>& globalSize, std::vector<unsigned int>& localSize)
{
  static bool msg_printed = false;
  unsigned int totalLocal = 1;
  unsigned int totalGlobal = 1;
  ...
  unsigned int workgroupCount = std::max(1U, (totalGlobal / totalLocal) );

  unsigned int workgroupBufferSize = totalLocal * getWorkItemPrintfBufferSize();
  unsigned int retval = workgroupCount * workgroupBufferSize;
