...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        PerfMon perfmon(&proxy);
        
        // Transfer data (max size is 8 MB)
        if (transferSizeTest(proxy, alignment, 0x7D0000) != 0) {
            std::cout << "ERROR 1: Data Transfer Failed\n";
            std::cout << "FAILED TEST\n";
            return 1;
        }

        // Read performance results
        if (perfmon.readTrace(XCL_PERF_MON_MEMORY) != 0) {
            std::cout << "ERROR 2: APM Trace in base platform is empty\n";
            std::cout << "FAILED TEST\n";
            return 2;
        }
        
        if (perfmon.readTrace(XCL_PERF_MON_OCL_REGION) != 0) {
            std::cout << "ERROR 3: APM Trace in OCL region is empty\n";
            std::cout << "FAILED TEST\n";
            return 3;
        }
    }
