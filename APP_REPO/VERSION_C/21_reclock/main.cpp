...
/**
 * Tests OCL Region clock scaling feature of hardware and driver.
 * Runs an OpenCL kernel global-local memory bandwidth test with OCL running
 * at regular and lower frequencies. We should see reduced bandwidth for the
 * latter.
 */
...
{
    // TODO: Move the buffer allocation out of runKernel
    ...
    // Use a different pattern every run so we can be sure that the output is indeed
    // generated by this run.
    static char mark = 0x66;
    std::memset(dataBuffer.getBuffer(), mark++, dataBuffer.size());
    ...
    proxy.freeDevice(deviceResult);
    proxy.freeDevice(deviceData);
    return 0;
}
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        const unsigned initialFreq = info.mOCLFrequency;
        for (unsigned freq = initialFreq; freq >= 25; freq -= 10) {
            std::cout << "Requesting Kernel Frequency = " << freq << "MHz\n";
            if (proxy.scaleFrequency(freq)) {
                std::cout << "FAILED TEST\n";
            ...
            std::cout << "Kernel Frequency = " << info.mOCLFrequency << "MHz\n";
            if (runKernel(proxy, alignment, verbose)) {
                std::cout << "FAILED TEST for frequency " << info.mOCLFrequency << "\n";
                return 1;
            }
        }

        // Restore the frequency
        if (proxy.scaleFrequency(initialFreq)) {
            std::cout << "FAILED TEST\n";
            std::cout << "Unable to change clock frequency\n";
            return 1;
        }
