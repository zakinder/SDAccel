...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        // BlockSize = 16 KB, 262144 blocks
        if (transferBenchmarkTest(proxy, alignment, 0x4000, 262144) != 0) {
            std::cout << "FAILED TEST\n";
            return 1;
        }

        // BlockSize = 256 KB, 16384 blocks
        if (transferBenchmarkTest(proxy, alignment, 0x40000, 16384) != 0) {
            std::cout << "FAILED TEST\n";
            return 1;
        }

        // BlockSize = 8 MB, 512 blocks
        if (transferBenchmarkTest(proxy, alignment, 0x800000, 512) != 0) {
            std::cout << "FAILED TEST\n";
            return 1;
        }

        // BlockSize = 16 MB, 256 blocks
        if (transferBenchmarkTest(proxy, alignment, 0x1000000, 256) != 0) {
            std::cout << "FAILED TEST\n";
            return 1;
        }

        // BlockSize = 64 MB, 64 blocks
        if (transferBenchmarkTest(proxy, alignment, 0x4000000, 64) != 0) {
            std::cout << "FAILED TEST\n";
            return 1;
        }


        // BlockSize = 128 MB, 32 blocks
        if (transferBenchmarkTest(proxy, alignment, 0x8000000, 32) != 0) {
            std::cout << "FAILED TEST\n";
            return 1;
        }


        // BlockSize = 256 MB, 16 blocks
        if (transferBenchmarkTest(proxy, alignment, 0x10000000, 16) != 0) {
            std::cout << "FAILED TEST\n";
            return 1;
        }
    }
